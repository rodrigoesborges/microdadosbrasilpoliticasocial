...
n_ind_sa$leg <- c("mortes maternas no parto a cada 1.000 partos",
                  ...
                  "Fecundidade adolescente ( /1.000 adolescentes 15 a 19)",
...
indicadores_saude$indicador <- as.factor(indicadores_saude$indicador)

nom_ind_s <- c("Fecundidade Adolescente (Nascidos de Adolescentes \npor 1.000 adolescentes na faixa de 15 a 19 anos)",
               "Proporção de gestações com número insuficiente \nde consultas pré-natais (1 a 6 consultas)",
               "Taxa de mortalidade materna",
               "Óbitos maternos a cada 100.000 mulheres em idade fértil",
               "Proporção de nascidos em hospital",
               "Proporção de gestações com número adequado \nou ótimo de consultas pré-natais (6 ou mais consultas)",
               "Proporção dos partos cesários em relação ao total de partos",
               "Proporção de gestações sem nenhuma consulta pré-natal"
)

levels(indicadores_saude$indicador) <- nom_ind_s
...
saude_mr_uf <- indicadores_saude %>% group_by(ano, indicador, uf) %>% 
  summarise(valor = weighted.mean(value, pop0a6, na.rm = T), Microrregião = as.character(max(uf)),
            cod_mr = max(uf)*1000)
...
saude_mr[saude_mr$cod_mr == 0,]$Microrregião <- "Brasil"
...
saude_mr_es[saude_mr_es$Microrregião == 32,]$Microrregião <- "Espírito Santo"

saude_mr_es$indicador <- as.character(saude_mr_es$indicador)
