...
mortes_parto <- rbindlist(lapply(anosel,
                             function(x) data.frame(ano = x, sim_mat10_mun(periodo = x, conteudo = 2),
                                                    indicador ="Mortes relativas ao parto/Maternos", stringsAsFactors = F))) %>%
...
#indicador de mortes no parto por 100.000 mulheres na idade fértil
...
mortes_parto_ind$mortes_por_1000 <- 100000*mortes_parto_ind$Óbitos.maternos/mortes_parto_ind$mulheresf

#Indicador de mortes maternas a cada 100.000  nascidos vivos
#Estimativa de nascidos vivos

nasc_v <- rbindlist(lapply(anosel,
                                      function(x) data.frame(ano = x, sinasc_nv_mun(periodo = x, coluna = "Tipo de gravidez"), 
                                                             stringsAsFactors = F))) %>%
  separate(Município,into = c("cod_mun","Município"), sep = " ",extra = "merge", fill = "right") 
nasc_v <- nasc_v %>% 
  transmute(cod_mun = cod_mun, ano = ano, Município = Município, estimativa_partos = Total )
            #- Aparentemente retirar essas gestações reduziu muito o indicador - Dupla - 2*Tripla.e.mais)
#  gather(indicador, valor, c(-1,-2,-3)) %>% mutate(indicador = paste("Gravidez ",indicador))

mortes_parto_ind <- mortes_parto_ind %>% left_join(nasc_v, by = c("cod_mun", "Município", "ano")) 


#colocar 1 nascido para os NAs - provisório pensar melhor depois
mortes_parto_ind[is.na(mortes_parto_ind$estimativa_partos),]$estimativa_partos <- 1


mortes_parto_ind$mortes_por_100k_nv <- 100000*mortes_parto_ind$Óbitos.maternos/mortes_parto_ind$estimativa_partos

mortes_parto_ind <- transmute(mortes_parto_ind, cod_mun = cod_mun, Município = Município, ano = ano, 
                              mortes_100k_nv = mortes_por_100k_nv, mortes_por_1000_m = mortes_por_1000)
...
m_id_10a19 <- popmf_det[,c(1,2,3,6,9,10,11)] %>% transmute(ano = ano, cod_mun = cod_mun, m_10a_19 = `15 a 19 anos`)
#Retirada parte de 10 a 14 =>  taxa de fecundidade adolescente se mede de 15 a 19 =>  `10 a 14 anos`+
...
fecundidade_adolescente <- nasc_v_idadem %>% 
  filter(faixa_etaria_mae %in% c("15 a 19 anos")) %>%
  # retirado 10 a 14 anos "10 a 14 anos"
  pivot_wider(names_from = faixa_etaria_mae,values_from = valor) %>% 
  transmute(ano = ano, cod_mun = cod_mun, Município = Município, `f. 10 a 19 anos` = `15 a 19 anos`) %>%
  #retirado 10 a 14 anos `10 a 14 anos`+
