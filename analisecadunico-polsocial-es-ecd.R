...
calc_ind_cu <- function(ano = 2017, uf = 32) {
    ...
    #dbGetQuery( db , "SELECT stype , dnum , cname , SUM( pw ) FROM apiclus1 GROUP BY stype , dnum , cname" ) )
      ecdpbfmun <- svyby(~I(marc_pbf == 1),~cd_ibge,cadunico_ecd, svytotal,na.rm = TRUE )
      ...
      ecdpbfmun <- ecdpbfmun %>% right_join(popm_inf[popm_inf$ano == ano  & grepl(sprintf("%s",ufc),popm_inf$cod_mun),c(1,2,7)], by = "cod_mun") 
