...
ind_ecd_ed <- function (a = 2017, tbase = popm_inf) {
  ...
  #D) n_matriculas_creche - criancas da idade certa
 ...
 e_1_pub <- escolas[(IN_COMUM_CRECHE == 1 | IN_ESP_EXCLUSIVA_CRECHE == 1) & TP_DEPENDENCIA != 4,] %>% count(CO_MUNICIPIO)
 
  
  #J) Pré-escolas públicas por município
 
  e_2_pub <-  escolas[(IN_COMUM_PRE == 1 | IN_ESP_EXCLUSIVA_PRE == 1) & TP_DEPENDENCIA != 4,] %>% count(CO_MUNICIPIO)
  ...
  r_bruto <- m_1_mun %>% full_join(m_1_mun_ef, by = m) %>% full_join(m_2_mun, by = m) %>% full_join(m_2_mun_ef, by = m) %>% 
    ...
    full_join(e_1_pub, by = m)%>% full_join(e_2_mun, by = m) %>% full_join(e_2_pub, by = m)
  ...
  names(r_bruto) <- c("cod_mun","m_1_mun","m_1_mun_ef","m_2_mun","m_2_mun_ef","al_1_sala_mun","al_2_sala_mun",
                      "n_creches","n_creches_pub","n_pre","n_pre_pub")
